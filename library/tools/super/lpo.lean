...
private meta def prec_gt_of_name_list' (nis : rb_map name ℕ) : expr → expr → bool :=
λs t,
match (rb_map.find nis (name_of_funsym s), rb_map.find nis (name_of_funsym t)) with
...
private meta def mk_lpo' (gt : expr → expr → bool) : expr → expr → bool :=
lpo gt
