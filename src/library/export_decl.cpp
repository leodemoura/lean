...
environment add_export_decl_core(environment const & env, name const & in_ns, export_decl const & e) {
...
environment add_export_decl(environment const & env, export_decl const & entry) {
    name in_ns = get_namespace(env);
    environment new_env = add_export_decl_core(env, in_ns, entry);
    return module::add(new_env, std::make_shared<export_decl_modification>(in_ns, entry));
}
