...
static optional<tactic_state> apply(type_context & ctx, expr e, apply_cfg const & cfg, tactic_state const & s,
                                    vm_obj * out_error_obj, list<expr> * all_metas) {
    ...
    if (all_metas) *all_metas = to_list(new_goals);
...
vm_obj tactic_apply_core(vm_obj const & e, vm_obj const & cfg0, vm_obj const & s0) {
    ...
    try {
        vm_obj error_obj;
        list<expr> all_metas;
        optional<tactic_state> new_s = apply(ctx, to_expr(e), cfg, s, &error_obj, &all_metas);
        if (!new_s)
            return error_obj;
