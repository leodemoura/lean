...
struct vm_decls : public environment_extension {
    ...
    unsigned reserve(name const & n, unsigned arity) {
        unsigned idx = get_vm_index(n);
        if (auto decl = m_decls.find(idx)) {
            if (decl->get_arity() != arity)
                throw exception(sstream() << "VM already contains '" << n << "', but registered with a different arity (new: " << arity << ", old: " << decl->get_arity() << ")");
        }
