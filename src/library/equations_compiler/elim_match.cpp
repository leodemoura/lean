...
static bool is_value(type_context & ctx, expr const & e) {
    try {
...
static expr whnf_pattern(type_context & ctx, expr const & e) {
...
static expr whnf_constructor(type_context & ctx, expr const & e) {
    return ctx.whnf_head_pred(e, [&](expr const & e) {
...
static optional<name> is_constructor_app(type_context & ctx, expr const & e) const {
...
static bool is_finite_value(type_context & ctx, expr const & e) {
...
struct elim_match_fn {
    ...
    format pp_problem(problem const & P) {
        format r;
        type_context ctx = m_ectx.mk_cacheless_type_context(get_local_context(P));
    ...
    unsigned get_eqns_arity(local_context const & lctx, expr const & eqns) {
        ...
        type_context ctx = m_ectx(lctx);
        unpack_eqns ues(ctx, eqns);
        unsigned r = ues.get_arity_of(0);
        m_ectx.restore(ctx);
        return r;
    }
    ...
    bool is_constructor_transition(problem const & P) {
        return all_equations(P, [&](equation const & eqn) {
                ...
                type_context ctx = m_ectx.mk_type_context(eqn.m_lctx);
                bool r;
    ...
    */
    bool is_value_transition(problem const & P) {
        ...
        if (!has_variable && has_finite_value)
            return false;
        type_context ctx = m_ectx.mk_type_context(get_local_context(P));
        ...
        for (unsigned i = 0; i < values.size(); i++) {
            ...
            for (equation const & eqn : P.m_equations) {
                ...
                if (p == val) {
                    ...
                    new_eqn.m_lhs_args = map(eqn.m_lhs_args, [&](expr const & arg) { return replace_locals(arg, from, to); });
                    new_eqn.m_rhs      = replace_locals(eqn.m_rhs, from, to);
                    new_eqn.m_patterns = map(tail(eqn.m_patterns), [&](expr const & p) { return replace_locals(p, from, to); });
                    new_eqns.push_back(new_eqn);
                    m_ectx.restore(ctx);
                    if (is_last) break;
                }
...
eqn_compiler_result mk_nonrec(equations_context & ectx, expr const & eqns) {
    ...
    */
    ...
    fn         = p.second;
    unsigned eqn_idx     = 1;
    type_context ctx = m_ectx.mk_type_context();
